...
render_adapter/               # Node — vendor-agnostic styling interface (repo root)
  ...
  tests/
ui/                            # Python — local FastAPI test UI (repo root)
  server.py                     # thin wrapper over vastu_audit/suggestions + auth-gated /flats* endpoints
  storage.py                     # SQLite: users, sessions, flats, flat_versions
  auth.py                        # bcrypt password hashing + session-token helpers
  index.html                     # single-page vanilla HTML/JS/CSS UI, no build step
  tests/
docs/
...
| `ui/storage.py` (projects) | Done — `projects`/`project_versions`/`project_proposals` tables so one building's confirmed geometry (boundary, facade bearing, base room layout) is captured once and reused by every flat owner in it, instead of each tracing from scratch. Submit (no auth) → admin approve/reject (`ui/admin.html`, gated by a fail-closed `VASTU_ADMIN_TOKEN` shared secret — 503 if unconfigured, not silently open) → live. Every live version traces to exactly one approved proposal; rejected proposals are kept, not deleted. 24 storage tests + 12 server integration tests. Found and fixed two real bugs along the way: a route-ordering conflict (`/projects/{id}` swallowing `/projects/proposals`) and a latent SQLite cross-thread `.close()` crash under FastAPI's threadpool (fixed via `check_same_thread=False`, safe since each connection is still only ever used by one request at a time) |
...
| `genesis/engine/solver.py` | Done (Phase 1c) — constraint-based placement for bed (MasterBedroom/GuestBedroom/ChildrenBedroom), stove (Kitchen), and heavy-furniture wall recommendation (LivingRoom) only; always returns a least-bad placement with a `compromise`/`compromise_note` flag rather than silently failing or violating a rule, 13 tests. Does not attempt general room layout or any other room type. Also exposes a public `nearest_wall()` helper (wall/quadrant lookup for an arbitrary point), added for `furniture_detection_adapter.py` to reuse without duplicating the wall-finding geometry |
| `genesis/engine/furniture_detection_adapter.py` | Done (OPT-IN, decoupled) — translates planparser-class ML furniture-detection output (`{"class", "bbox", "confidence"}` records) into a satisfies-rule verdict for ALREADY-DETECTED furniture, checked against the same preferred/forbidden directions `solver.py`/`vastu_audit.py` already define. Covers only bed (MasterBedroom/GuestBedroom/ChildrenBedroom), stove (Kitchen), and sofa/heavy-furniture (LivingRoom) — the three types with an established wall-flush-to-direction interpretation; every other detected class (toilet, chair, table, sink, door, shower, bathtub, vanity) is explicitly left uninterpreted rather than guessed. Confidence-gated (`min_confidence`, default 0.5) and wall-flush-gated (`wall_flush_margin_ft`, default 2.0 ft); every input detection lands in `checks` or a named `skipped` bucket, never silently dropped. Answers "does the existing (detected) item already satisfy the rule", not "where should it go" — `solver.py` never imports this module, and this module never calls any `solve_*` function or feeds a recommendation back into the solver. 14 tests |
...
| `ui/` | Done (local test UI) — FastAPI wrapper over `vastu_audit.audit_layout`/`suggestions.suggest_for_layout`, unchanged; the stateless `POST /audit` needs no login. Real user accounts replaced the old free-text `owner` label: bcrypt-hashed passwords (`auth.py`), server-side sessions via an httponly cookie (`storage.sessions`, revoked immediately on logout — not a stateless/signed token), and every `/flats*` endpoint scoped to the authenticated user (`storage.py`'s flat functions filter by `user_id` in SQL, so another user's flat 404s exactly like a nonexistent one). `storage.py` gained `users`/`sessions` tables and indexes on the foreign keys, and refuses to open a pre-auth-schema SQLite file with a clear `RuntimeError` rather than silently degrading. `index.html` is still a single no-build-step page (vanilla HTML/JS), restyled (CSS custom properties, card layout, responsive) with login/register screens gating the app; the tracing tool, project picker, shape-diagnosis panel, and ritual-protocol UI (see the `ui/storage.py` (projects) row below) are reconciled into the same restyle and remain functionally unchanged — verified together end-to-end in a headless-Chromium smoke test (register, trace a boundary, audit, save, reload, admin page). Project proposals stay intentionally free-text-submitted (no login required), separate from the new user-account system — see `ui/server.py`'s `require_admin` docstring for why the two gates are kept independent. 93 tests total across `test_storage.py`/`test_server.py`/`test_auth.py` |
