...
2. **IMPLEMENTED.** `genesis/engine/furniture_detection_adapter.py` — translates planparser-class ML detections (`{"class", "bbox", "confidence"}`) into a satisfies-rule verdict for furniture a detector found ALREADY PRESENT in a room, checked against the exact preferred/forbidden directions `solver.py`/`vastu_audit.py` already define. Scoped narrowly to the three furniture types with an established wall-flush-to-direction interpretation — bed (all three bedroom types), stove (Kitchen), sofa/heavy-furniture (LivingRoom) — every other planparser class (toilet included, despite `vastu_audit` having a `Toilet.seat_direction` rule: its "faces away from the wall" convention is genuinely different from bed/stove's "faces toward the wall" one and was deliberately not guessed at) is passed through uninterpreted. Confidence- and wall-flush-gated, with every input detection landing in `checks` or a named `skipped` bucket so nothing is silently dropped. `solver.py` gained one small additive public helper (`nearest_wall()`) so the adapter can reuse its wall/quadrant geometry instead of duplicating it; no existing `solve_*` function changed, and `solver.py` does not import the adapter — it only answers "does the existing item already satisfy the rule", never "where should it go". 14 new tests, all passing; full suite now 223 tests.
...
`docs/build_plan.md` was updated for both implemented items (1 and 2); items 3-5 remain unscheduled design spikes / deferrals, not yet sized into build-plan rows.
